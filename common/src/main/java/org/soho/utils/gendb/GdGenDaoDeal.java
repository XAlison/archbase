...
public class GdGenDaoDeal {

/*	public static void main(String[] args) throws IOException {
		File file = new File(Constants.GDAO);
		GdGenDaoDeal gen = new GdGenDaoDeal();
		gen.deal(file);
	}*/
	...
	public void genDao(BeanElement bean) throws IOException {
		...
		out.println("public class " + className + "Dao extends BaseDao<"
				+ className + "," + StringUtil.upperFirst(pkType) + ">{");
		...
		sb = new StringBuffer(" 		DELETE_SQL = \"DELETE FROM \"+")
				.append("TABLE_NAME").append("+\" WHERE ").append(pk)
				.append("=?\";");
		out.println(sb.toString());
		...
		for (int i = 0; i < pors.size(); i++) {
			...
			sb.append(ac.column).append("=:")
					.append(StringUtil.lowerFirst(ac.attr));
		}
		sb.append(" where ");
		sb.append(pkcolumn).append("=:").append(StringUtil.lowerFirst(pk))
				.append("\";");

		out.println(sb.toString());

		sb = new StringBuffer(" 		mapper = new SingleColumnRowMapper<")
				.append(className).append(">() {\n");
		sb.append("\t\t\t\tpublic ")
				.append(className)
				.append("  mapRow(ResultSet rs, int rowNum) throws SQLException { \n");
		sb.append("\t\t\t\t\t").append(className).append(" ")
				.append(StringUtil.lowerFirst(className)).append(" =  new ")
				.append(className).append("();\n");
		String voName = StringUtil.lowerFirst(className);
		...
		for (int i = 0; i < pors.size(); i++) {
			...
			sb.append("\t\t\t\t\t").append(voName).append(".set")
					.append(StringUtil.upperFirst(attr)).append("(rs.get");
